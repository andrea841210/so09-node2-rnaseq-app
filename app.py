...
def prepare_fgsea(df_fgsea_raw: pd.DataFrame) -> pd.DataFrame:
    ...
    # common fgsea result fields normalization
    ...
    if "pathway" not in df.columns:
        # try to infer first string column as pathway
        for c in df.columns:
...
def prepare_rank(df_rank_raw: pd.DataFrame) -> pd.DataFrame:
    ...
    # candidate column names for ranking score
...
def prepare_escurve(df_es_raw: pd.DataFrame) -> pd.DataFrame:
    ...
    # expecting columns like position / running_es / hit
    # we keep flexible; the plotting section will check columns safely
    return df
...
if show_table_sample:
    with st.expander("Input snapshots", expanded=False):
        if df_deg is not None:
            st.write("DEG table:", df_deg.head())
        if df_fgsea is not None:
            st.write("fgsea_results:", df_fgsea.head())
        if df_rank is not None:
            st.write("rank.csv:", df_rank.head())
        if df_le is not None:
            st.write("leading_edge.csv:", df_le.head())
        if df_es is not None:
            st.write("es_curve.csv:", df_es.head())
...
with tab1:
    ...
    if df_deg is None:
        st.info("Upload DEG table to render volcano.")
    else:
        try:
            ...
            ax.axvline(+logfc_cutoff, ls="--", lw=1)
            ax.axvline(-logfc_cutoff, ls="--", lw=1)
...
with tab2:
    ...
    if df_deg is None:
        st.info("Upload DEG table to render MA plot.")
    else:
        try:
            ddeg = prepare_deg(df_deg)
            ...
            ax.set_xlabel("Abundance (baseMean / aveExpr)")
...
with tab3:
    ...
    with col_g1:
        if df_fgsea is None:
            st.info("Upload fgsea_results.csv to show pathway table.")
        else:
            try:
    ...
    with col_g2:
        ...
        if df_es is None:
            st.info("Upload es_curve.csv to render ES curve.")
        else:
            try:
                des = prepare_escurve(df_es)
                # Flexible column name inference
                pos_col = None
                for c in ["position", "pos", "i", "rank_index"]:
                    if c in des.columns:
                        pos_col = c; break
                es_col = None
                for c in ["running_es", "running_score", "es", "score"]:
                    if c in des.columns:
                        es_col = c; break
...
with tab4:
    ...
    if df_le is None:
        st.info("Upload leading_edge.csv to list genes.")
    else:
        try:
...
with st.container():
    ...
    if st.button("Build ZIP"):
        bufs = []
        names = []
        # add raw inputs if present
        for file_obj, nm in [(up_deg, "DEG.csv"), (up_fgsea, "fgsea_results.csv"), (up_rank, "rank.csv"), (up_le, "leading_edge.csv"), (up_es, "es_curve.csv")]:
            if file_obj is not None:
                file_obj.seek(0)
                bufs.append(file_obj.read())
                names.append(nm)
        # simple pack
        import zipfile
        bio = io.BytesIO()
        with zipfile.ZipFile(bio, mode="w", compression=zipfile.ZIP_DEFLATED) as zf:
            for data, nm in zip(bufs, names):
                zf.writestr(nm, data)
        st.download_button(label="Download ZIP", data=bio.getvalue(), file_name=fn, mime="application/zip")

# Footer
st.markdown("---")
st.caption("Reset now fully clears uploaded anchors via reset_id-salted keys and cache purge.")
